          ...
          if (line_count == 2) {
              ...
              while ((pos = line.find(delimiter)) != std::string::npos) {
                  ...
                  if (count == 1) {
                      ...
                      inet_pton(AF_INET, token.c_str(), &(server_addr.sin_addr));
                      server_addr.sin_port = htons(std::stoi(line.substr(pos + 1)));
                      ...
                      if (server.client_fd != -1) {
                          ...
                          int activity = select(server.client_fd + 1, NULL, &writefds, NULL, &timeout);
                          if (activity != -1 && FD_ISSET(server.client_fd, &writefds)) {
                              if (connect(server.client_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) == -1) {
              ...
              // Extract shared memory pathname and proj_id
              delimiter = " ";
              pos = 0;
              count = 0;
              ...
              while ((pos = line.find(delimiter)) != std::string::npos) {
                  ...
                  if (count == 1) {
                      ...
                      key_t key = ftok(token.c_str(), std::stoi(line.substr(pos + 1)));
                      server.shmid = shmget(key, SHARED_MEMORY_SIZE, 0666);
                    }
                  line.erase(0, pos + delimiter.length());
                  count++;
                }
...
void run(const std::string& client_files_directory) {
  ...
  if (num_servers == 0) {
      std::cout << "No servers found in the directory: " << client_files_directory << "\n";
      return;
    }
